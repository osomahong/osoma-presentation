...
def s_cover(n):
    ...
    body = (f'{aur}{sig}<div class="fg">'
            ...
            '<p class="sub">네 해 동안 무엇이 쌓였고, 그것을 지금 어떻게 굴리고 있는지 순서대로 봅니다.</p>'
...
def s_intro(n):
    ...
    return split(n, P1, "회사", '등본에 적힌 <span class="gt">오픈소스마케팅</span>',
                 "등본에 적힌 오픈소스마케팅",
                 "업종 네 가지가 회사의 성격을 그대로 보여 줍니다. 컨설팅과 광고, 교육, 그리고 소프트웨어이고, "
                 "실제로 한 일도 이 넷을 벗어나지 않았습니다.",
                 points, _info(rows), "")
...
def s_did_what(n):
    ...
    inner = ('<div class="vcen">'
             ...
             + band('네 갈래가 따로 팔린 적은 거의 없습니다. 분석을 깔고 그 위에서 마케팅을 돌리는 한 덩어리였고, '
                    '<strong>맨 앞에 놓이는 분석 환경 구축부터 봅니다.</strong>')
             + '</div>')
    return wide(n, P1, "한 일의 갈래", '일로 보면 <span class="gt">네 갈래</span>입니다',
                "우리가 실제로 한 네 갈래의 일",
                "앞 장의 업종 넷이 일로는 이렇게 나옵니다. 2022년부터 2025년까지의 프로젝트를 성격으로 묶었습니다.",
                inner, "")
...
def s_did_steps(n):
    ...
    inner = ('<div class="vcen">' + howto(flow, facts)
             + band('무엇을 셀지 먼저 정하지 않으면 태깅은 매번 다시 하게 됩니다. '
                    '<strong>분석은 이렇게 순서에서 시작하지만, 그로스 마케팅은 문제에서 시작합니다.</strong>')
             + '</div>')
    return wide(n, P1, "일하는 순서", '정하고, 적고, 걸고, <span class="gt">검수하고</span>, 넘깁니다',
                "구축이 지나는 여섯 단계",
                "앞 장의 여섯 가지가 이 순서를 지나며 하나씩 나옵니다.",
                inner, "")
...
def s_did_growth(n):
    ...
    inner = ('<div class="vcen">'
             ...
             + band('전략 문서만 넘기고 끝난 건은 드뭅니다. 매체를 함께 돌리고 결과를 읽는 데까지 붙어 있었고, '
                    '<strong>그렇게 붙어 있던 자리가 네 해 동안 53건이 되었습니다.</strong>')
             + '</div>')
    return wide(n, P1, "그로스 마케팅", '풀어 달라고 <span class="gt">불려 온 자리</span>들입니다',
                "그로스 마케팅에서 푸는 문제",
                "여기서는 정해진 순서가 아니라 그 회사가 지금 막힌 지점에서 출발합니다.",
                inner, "")
...
def s_hist(n):
    ...
    return wide(n, P1, "연혁", '네 해 동안 <span class="gt">53건</span>이 쌓였습니다',
                ...
                _rows(rows) + band('해가 갈수록 건수보다 업종의 폭이 넓어졌습니다. '
                                   '<strong>이 53건을 누구와 했는지가 다음 이야기입니다.</strong>'), "")
...
def s_ind_overview(n):
    ...
    inner = (_facts(items)
             + band('한 갈래에 기대어 온 회사가 아닙니다. '
                    '<strong>일하는 모양이 가장 정형화된 갈래부터 봅니다.</strong>'))
    return wide(n, P2, "산업군 개요", '한 업종에 <span class="gt">묶이지 않았습니다</span>',
                "고객사를 네 갈래로 묶으면",
                "앞 장의 53건을 고객사 성격으로 다시 세면 네 갈래가 나옵니다. 대기업 유통부터 대학과 정부기관, 스타트업까지 섞여 있습니다.",
                inner, "")
...
def s_ind_retail(n):
    ...
    inner = (_rows(rows, "grp")
             + band('웹과 앱을 함께 쓰고 구매가 성과라는 점이 같아 이벤트 정의서의 뼈대가 서로 크게 다르지 않습니다. '
                    '<strong>성과를 정하는 일부터 어려운 쪽은 그다음 갈래입니다.</strong>'))
    return wide(n, P2, "산업군", '<span class="gt">유통과 커머스</span>가 가장 익숙합니다',
                "유통과 커머스 열두 곳", "12건입니다. 넷 가운데 가장 오래 해 온 쪽입니다.", inner, "")
...
def s_ind_it(n):
    ...
    inner = (_rows(rows, "grp")
             + band('B2B가 많아 성과가 구매가 아니라 문의와 상담입니다. '
                    '<strong>한 번으로 끝나지 않은 곳들은 그다음 갈래에 모여 있습니다.</strong>'))
    return wide(n, P2, "산업군", '성과를 정하기 어려운 쪽은 <span class="gt">IT와 금융</span>입니다',
                "IT와 금융 열세 곳",
                "13건입니다. 구매가 없는 곳이 많아 무엇을 성과로 볼지부터 정해야 했습니다.", inner, "")
...
def s_ind_public(n):
    ...
    inner = (_rows(rows, "grp")
             + band('한국관광공사 지원사업은 2024년부터 세 해를 이어서 맡고 있습니다. '
                    '<strong>같은 발주처가 다시 맡겼다는 것이 이 갈래의 성적입니다.</strong>'))
    return wide(n, P2, "산업군", '<span class="gt">공공과 대학</span>은 한 번으로 끝나지 않습니다',
                "교육과 공공 열 곳",
                "10건으로 가장 적지만, 같은 곳을 여러 해 맡은 사례가 여기에 모여 있습니다.", inner, "")
...
def s_ind_rest(n):
    ...
    rows = [("제조와 건설", "오프라인이 본업", [("경동나비엔", "on"), ("위닉스", ""),
            ...
            ("그 밖", "초기 단계와 광고", [("남유 FNC", ""), ("빅픽처팀", ""), ("티니어", ""),
                                    ("뭉클랩", ""), ("에프엠커뮤니케이션", ""), ("비플레인", "")])]
    inner = (_rows(rows, "grp")
             + band('업종이 이만큼 벌어지면 사람 수로는 감당이 되지 않습니다. '
                    '<strong>스물네 곳을 한꺼번에 맡을 때 그 방법이 가장 잘 드러납니다.</strong>'))
    return wide(n, P2, "산업군", '하나로 묶기 어려운 <span class="gt">열여덟 곳</span>이 남습니다',
                "제조와 서비스, 비영리 열여덟 곳",
                "18건으로 넷 가운데 가장 많습니다. 업종이 제각각이라 성과 정의도 매번 새로 했습니다.",
                inner, "")
...
def s_kto_setup(n):
    ...
    return split(n, P3, "판 깔기", '먼저 <span class="gt">자리를 정합니다</span>',
                 "사업을 여는 순간 정하는 것",
                 "사업이 열리면 가장 먼저 하는 일은 컨설팅이 아니라 자리를 정하는 일입니다. 여기서 정한 자리가 넉 달을 버팁니다.",
                 points, _info(rows), "")
...
def s_kto_sched(n):
    ...
    return wide(n, P3, "일정 관리", '모으지 않고 <span class="gt">쌓이게</span> 둡니다',
                "스물네 곳 일정이 엉키지 않는 이유",
                "자리를 정했으면 다음은 일정입니다. 회차 관리는 사업 운영에서 가장 자주 깨집니다.",
                inner, "")
...
def s_kto_weekly(n):
    ...
    inner = ('<div class="vcen">' + howto(flow, facts)
             + band('두세 줄이라 매주 채워지고, 매주 채워지니 월간 보고가 저절로 만들어집니다. '
                    '<strong>다만 두세 줄과 함께 적는 숫자 하나를 먼저 맞춰 두어야 합니다.</strong>')
             + '</div>')
...
def s_kto_rate(n):
    ...
    return split(n, P3, "진행률", '<span class="gt">70퍼센트</span>가 무엇인지 정해 둡니다',
                 "진행률을 숫자로 맞추는 법",
                 "주간 보고에 함께 적는 그 숫자입니다. 적는 사람마다 다르게 쓰므로 단계마다 뜻을 정해 둡니다.",
                 points, _info(rows), "")
...
def s_kto_report(n):
    ...
    inner = ('<div class="vcen">'
             ...
             + band('주간 요약이 월간 보고가 되고, 차수별 결과보고서가 최종 성과가 됩니다. '
                    '<strong>세 층이 같은 자료를 쓰는데, 넉 달 사이 그 자료의 기준이 바뀌기도 합니다.</strong>')
             + '</div>')
    return wide(n, P3, "성과 보고", '주간, 월간, <span class="gt">차수별</span>로 나눕니다',
                "공사에 보고하는 세 층",
                "주간에 쌓인 것이 위로 어떻게 올라가는지 봅니다. 주기마다 문서를 새로 만들면 스물네 곳을 감당할 수 없습니다.",
                inner, "")
...
def s_kto_change(n):
    ...
    inner = ('<div class="vcen">' + vs("흔한 방식", "이 사업에서 하는 방식", rows)
             + band('바뀌는 것을 막는 대신 바뀐 자리를 한곳에 남깁니다. '
                    '<strong>이 방식으로 세 해를 돌렸습니다.</strong>')
             + '</div>')
...
def s_kto_years(n):
    ...
    inner = (_rows(rows, "grp")
             + band('해마다 사업의 이름과 무게가 달라졌고, 세 번째 사업의 이름에는 AI가 들어왔습니다. '
                    '<strong>같은 자리에서 맡는 일의 성격이 어떻게 달라졌는지가 다음 이야기입니다.</strong>'))
    return wide(n, P3, "운영 이력", '<span class="gt">세 해</span> 연속 운영사입니다',
                "세 해를 이어서 맡은 사업",
                "같은 발주처가 세 번 맡겼습니다. 연도별 성과 수치는 사업 종료 보고서에서 받아 채웁니다.",
                inner, "")
...
def s_ai_shift(n):
    ...
    return wide(n, P4, "달라진 것", '맡는 일이 <span class="gt">판단 쪽으로</span> 옮겨 왔습니다',
                "일의 성격이 바뀐 자리",
                "사업의 이름만 바뀐 것이 아닙니다. 같은 데이터를 다루지만 어디까지 맡는지가 달라졌습니다.",
                inner, "")
...
def s_ai_flow(n):
    ...
    inner = ('<div class="vcen">' + stepflow(steps, hot=(3,))
             + band('두 번째와 세 번째 단계에서 다른 회사와 갈립니다. '
                    '<strong>세 번째 단계에서 노트북을 열고 실제로 무엇을 까는지 먼저 봅니다.</strong>')
             + '</div>')
...
def s_ai_install(n):
    ...
    inner = ('<div class="vcen">' + _tools(items)
             + band('깔아 주지 않으면 GCP 키 발급에서 대부분 멈춥니다. '
                    '<strong>다만 무엇을 깔지는 기업마다 다릅니다.</strong>')
             + '</div>')
...
def s_ai_stack(n):
    ...
    inner = (_rows(rows, "grp")
             + band('도구를 정해 놓고 기업을 맞추지 않습니다. 무슨 일을 자동화할지 먼저 정하고 요금제를 고르는데, '
                    '<strong>이 고르는 일이 두 번째 단계에서 말한 구독 설계입니다.</strong>'))
    return wide(n, P4, "도구", '기업마다 <span class="gt">쓰는 것이 다릅니다</span>',
                "기업이 실제로 쓰는 도구",
                "같은 다섯 단계를 지나도 손에 쥐는 것은 다릅니다. 참여 기업이 고른 도구를 성격으로 묶었습니다.",
                inner, "")
...
def s_ai_sub(n):
    ...
    return split(n, P4, "구독 설계", '<span class="gt">무엇을 구독할지</span>도 컨설팅입니다',
                 "구독을 설계하는 일",
                 "앞 장의 도구를 한도 안에서 어떻게 조합할지 정하는 일입니다. 예산을 어디에 쓸지가 생각보다 큰 자리입니다.",
                 points, _info(rows), "")
...
def s_ai_us(n):
    ...
    inner = ('<div class="vcen">'
             ...
             + band('스물네 곳을 열한 명이 감당하는 힘이 여기서 나옵니다. '
                    '<strong>여기까지가 지금이고, 남은 네 장이 앞으로입니다.</strong>')
             + '</div>')
    return wide(n, P4, "내부 활용", '고객사에 권하기 전에 <span class="gt">먼저 씁니다</span>',
                "우리 일에도 AI를 씁니다",
                "여기까지가 고객사에 하는 일이고, 같은 것을 우리 업무 순서에도 넣었습니다.",
                inner, "")
...
def s_next_two(n):
    ...
    inner = ('<div class="vcen">'
             ...
             + band('둘은 따로 가는 길이 아닙니다. AX 컨설팅에서 반복되는 일이 다음 제품의 재료가 되기 때문인데, '
                    '<strong>각각 무슨 뜻인지 한 장씩 봅니다.</strong>')
             + '</div>')
    return wide(n, P5, "방향", '두 갈래로 <span class="gt">함께</span> 갑니다',
                "앞으로 갈 두 갈래",
                "지금까지가 해 온 일이라면, 여기부터는 어디에 무게를 옮길지입니다.",
                inner, "")
...
def s_next_ax(n):
    ...
    inner = ('<div class="vcen">' + stepflow(steps, hot=(2,))
             + band('AI를 도입했는지가 아니라 업무 시간과 결과물이 달라졌는지로 봅니다. '
                    '<strong>이 일을 하다 보면 매번 같은 도구를 다시 만들게 되는데, 그것이 두 번째 갈래입니다.</strong>')
             + '</div>')
...
def s_next_product(n):
    ...
    return wide(n, P5, "갈래 둘", '안에서 쓰던 것을 <span class="gt">밖으로</span> 냅니다',
                "도구를 제품으로 키우는 일",
                "매번 다시 만들던 것을 한 번 만들어 파는 물건으로 바꾸는 일입니다.",
                inner, "")
