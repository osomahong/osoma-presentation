...
사람이 읽을 수 있는 형태로만 다듬고 같은 캠페인의 표기 차이는 하나로 합친다.
뜻을 바꾸는 축약은 하지 않는다.

DROP_EXACT 와 ALWAYS_ON 은 고객사마다 달라서 비워 두었다. 프로젝트를 열 때 채운다.
"""
import difflib
import re
import unicodedata
...
# 고객사마다 다르므로 프로젝트를 열 때 실제 값을 보고 채운다.
DROP_EXACT = {
    "",
    "(not set)",
}

# 개별 캠페인이 아니라 상시로 열려 있는 창구. 유사도 병합에서 빼려고 따로 적어 둔다.
# 고객사마다 다르므로 프로젝트를 열 때 채운다.
ALWAYS_ON = set()
...
def clean(name: str) -> str | None:
    ...
    if name is None or "|" in name:  # "후원가이드 | 회사 이름" 같은 페이지 제목
        return None
    s = _tidy(name)
